...
$error['password'] = "Password không được trống";}if (empty($error)) {$is_check = $db->fetchOne("users"," email = '".$data['email']."' AND password = '".MD5($data['password'])."' ");if ($is_check != NULL) {$_SESSION['name_user'] = $is_check['name'];$_SESSION['name_id'] = $is_check['id'];echo "<script>alert('Đăng nhập thành công'); location.href='index.php'</script>";}else{$_SESSION['error'] = "Sai email hoặc mật khẩu";}}}?><?php require_once __DIR__. "/layout/header.php"; ?>
<div class="col-md-9 ">
